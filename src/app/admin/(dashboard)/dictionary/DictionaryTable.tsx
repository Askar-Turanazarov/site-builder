...
function Row({ entry }: { entry: Entry }) {
  ...
  return (
    <div className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] items-center gap-3 px-4 py-2.5">
      ...
      <input value={valueRu} onChange={(e) => setValueRu(e.target.value)} className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm text-ink outline-none focus:border-accent" />
      <input value={valueUz} onChange={(e) => setValueUz(e.target.value)} className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm text-ink outline-none focus:border-accent" />
      <input value={valueEn} onChange={(e) => setValueEn(e.target.value)} className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm text-ink outline-none focus:border-accent" />
      <div className="flex gap-1">
        <button
          type="button"
          onClick={save}
          disabled={!dirty || pending}
          className="rounded-xl border border-border px-2.5 py-1.5 text-xs font-medium text-ink hover:border-accent disabled:opacity-40"
        >
          {t("common.save")}
        </button>
  ...
  return (
    <div className="rounded-2xl border border-dashed border-border p-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        <input value={key} onChange={(e) => setKey(e.target.value)} placeholder="nav.contact" className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm outline-none focus:border-accent" />
        <select value={group} onChange={(e) => setGroup(e.target.value)} className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm outline-none focus:border-accent">
        ...
        <input value={valueRu} onChange={(e) => setValueRu(e.target.value)} placeholder="RU" className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm outline-none focus:border-accent" />
        <input value={valueUz} onChange={(e) => setValueUz(e.target.value)} placeholder="UZ" className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm outline-none focus:border-accent" />
        <input value={valueEn} onChange={(e) => setValueEn(e.target.value)} placeholder="EN" className="rounded-xl border border-border bg-paper px-2.5 py-1.5 text-sm outline-none focus:border-accent" />
      </div>
      <button
        type="button"
        onClick={submit}
