  ...
  return (
    <div className="mx-auto max-w-4xl px-8 py-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-ink">Страницы</h1>
          <p className="mt-1 text-sm text-muted">Страницы сайта: главная, о нас, услуги и другие.</p>
        </div>
        <Link
      ...
      <div className="mt-6 divide-y divide-border rounded-lg border border-border bg-surface">
        {pages.length === 0 && (
          <p className="px-5 py-8 text-center text-sm text-muted">Пока нет ни одной страницы.</p>
        )}
        {pages.map((page) => (
          <Link
            key={page.id}
          ...
          >
            <div>
              <div className="flex items-center gap-2">
                <span className="font-medium text-ink">{page.titleRu || "(без названия)"}</span>
                {page.isHomepage && (
