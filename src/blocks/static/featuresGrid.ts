...
export function featuresGridToHtml(data: BlockDataOf<"featuresGrid">): string {
  ...
  const cards = data.items
    .map((feature) => {
      ...
      return `
      <div>
        <span class="sb-icon mb-4 inline-flex h-11 w-11 items-center justify-center rounded-[var(--tpl-radius)]" style="background:var(--tpl-accent);color:var(--tpl-on-accent)">
