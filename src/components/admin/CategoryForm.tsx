...
export function CategoryForm({ initial }: { initial: CategoryFormInitial }) {
  ...
  function handleSave() {
    ...
    if (!slug.trim() || !name.ru.trim()) {
      setError("Укажите слаг и название (хотя бы на русском)");
      return;
    }
    startTransition(async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : "Не удалось сохранить рубрику");
      }
    });
  }
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : "Не удалось удалить рубрику");
      }
    });
  }
  ...
  return (
    <div className="mx-auto max-w-2xl px-8 py-10">
      <h1 className="font-display text-2xl font-semibold text-ink">
        {isNew ? "Новая рубрика" : "Редактирование рубрики"}
      </h1>
      ...
      <div className="mt-6 space-y-5 rounded-lg border border-border bg-surface p-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-muted">Слаг (URL)</label>
            <input
              type="text"
              value={slug}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-muted">Порядок сортировки</label>
            <input
              type="number"
              value={order}
        ...
        <LocalizedTextInput label="Название рубрики" values={name} onChange={(l, v) => setName((n) => ({ ...n, [l]: v }))} />
        <LocalizedTextInput label="Описание" values={desc} onChange={(l, v) => setDesc((d) => ({ ...d, [l]: v }))} multiline />
      </div>
